...
import BankTransferCheckout from "../components/BankTransfer";
...
export default function Register() {
  ...
  const validate = () => {
    ...
    if (!formData.bvn) newErrors.bvn = "Required";
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email))
      newErrors.email = "Invalid email";
    if (formData.password.length < 8)
      newErrors.password = "Minimum 8 characters";
    if (formData.subscription < 100)
      newErrors.subscription = "Minimum deposit $100";
    if (!formData.accreditation)
      newErrors.accreditation = "Required for compliance";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(
        "http://localhost:4000/investor/register",
        formData
      );
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {!transfer ? (
          ...
          <main className="flex-grow flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
            <div className="w-full max-w-md space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        className={`appearance-none relative block w-full px-3 py-2 border ${
                          errors.bvn ? "border-rose-300" : "border-slate-300"
                        } placeholder-slate-500 text-slate-900 rounded-md focus:outline-none focus:ring-amber-500 focus:border-amber-500 focus:z-10 sm:text-sm`}
                      />
                      {errors.bvn && (
                        <p className="mt-1 text-sm text-rose-600">
              ...
              <div className="mt-6 p-4 bg-slate-100 rounded-md text-xs text-slate-600">
                <p>
                  <strong>SEC Compliance:</strong> By registering, you
                  acknowledge Emex Capital LLC is a SEC-registered investment
                  advisor. All investors must verify accreditation status before
        ...
        <BankTransferCheckout storedUser={bankDetails.bank_account} amount={bankDetails.amount}/>
      )}
      {/* Footer */}
